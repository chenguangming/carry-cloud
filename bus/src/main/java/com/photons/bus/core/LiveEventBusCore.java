...
import com.photons.bus.ipc.receiver.LebIpcReceiver;
import com.photons.bus.utils.AppUtils;
import com.photons.bus.utils.ThreadUtils;
...
public final class LiveEventBusCore {
    ...
    private final Logger logger = LoggerFactory.getLogger("LiveEventBusCore");
    ...
    void registerReceiver() {
        ...
        if (application != null) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                application.registerReceiver(receiver, intentFilter, RECEIVER_NOT_EXPORTED);
            } else {
    ...
    private class LiveEvent<T> implements Observable<T> {
        ...
        private void postInternal(T value) {
            logger.debug("post: " + value + " with key: " + key);
            liveData.setValue(value);
        }

        @MainThread
        private void broadcastInternal(T value, boolean foreground, boolean onlyInApp) {
            logger.debug("broadcast: " + value + " foreground: " + foreground +
                    " with key: " + key);
            Application application = AppUtils.getApp();
            if (application == null) {
                logger.warn("application is null, you can try setContext() when config");
                return;
            }
        ...
        private void observeInternal(@NonNull LifecycleOwner owner, @NonNull Observer<T> observer) {
            ...
            logger.debug(observer + " observe " + key);
        }
        ...
        private void observeStickyInternal(@NonNull LifecycleOwner owner, @NonNull Observer<T> observer) {
            ObserverWrapper<T> observerWrapper = new ObserverWrapper<>(observer);
            liveData.observe(owner, observerWrapper);
            logger.debug(observer + " observe sticky " + key);
        }
        ...
        private void observeForeverInternal(@NonNull Observer<T> observer) {
            ...
            logger.debug(observer + " observe forever " + key);
        }
        ...
        private void observeStickyForeverInternal(@NonNull Observer<T> observer) {
            ...
            observerMap.put(observer, observerWrapper);
            liveData.observeForever(observerWrapper);
            logger.debug("observe sticky forever observer: " + observerWrapper + "(" + observer + ")"
                    + " with key: " + key);
        }
        ...
        private class LifecycleLiveData<T> extends ExternalLiveData<T> {
            ...
            public void removeObserver(@NonNull final Observer<? super T> observer) {
                ...
                logger.debug("observer removed: " + observer);
            }
    ...
    private class ObserverWrapper<T> implements Observer<T> {
        ...
        public void onChanged(@Nullable T t) {
            ...
            try {
                ...
                logger.warn("class cast error on message received: " + t, e);
            } catch (Exception e) {
                logger.error("error on message received: " + t, e);
            }
        }
    }
